...
function CraftOrders.initUI()
  ...
  window:createButton(splitter:partition(8), "Debug"%_t, "onDebug")
  window:createButton(splitter:partition(9), "Lazy Mine"%_t, "onLazyMineButtonPressed")
  window:createButton(splitter:partition(10), "Lazy Salvage"%_t, "onLazySalvageButtonPressed")
...
function CraftOrders.onDebug()
  if onClient() then
    invokeServerFunction("onDebug")
    ScriptUI():stopInteraction()
    return
  end

  Entity():addScriptOnce("data/scripts/lib/entitydbg.lua")
...
function CraftOrders.onLazyMineButtonPressed()
  if onClient() then
    invokeServerFunction("onLazyMineButtonPressed")
    ScriptUI():stopInteraction()
    return
  end
  ...
  if checkCaptain() then
    removeSpecialOrders()
    Entity():addScript("ai/lazymine.lua")
  end
end

function CraftOrders.onLazySalvageButtonPressed()
  if onClient() then
    invokeServerFunction("onLazySalvageButtonPressed")
    ScriptUI():stopInteraction()
    return
  end
  ...
  if checkCaptain() then
    removeSpecialOrders()
    Entity():addScript("ai/lazysalvage.lua")
  end
end


function CraftOrders.onMineAllButtonPressed()
  if onClient() then
    invokeServerFunction("onMineAllButtonPressed")
    ScriptUI():stopInteraction()
    return
  end
  ...
  if checkCaptain() then
    removeSpecialOrders()
    Entity():addScript("ai/mineall.lua")
  end
end


function CraftOrders.onLazyMineallButtonPressed()
  if onClient() then
    invokeServerFunction("onLazyMineallButtonPressed")
    ScriptUI():stopInteraction()
    return
  end
