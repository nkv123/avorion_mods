...
local ship = Entity()
local current
function AITransferFinish.getUpdateInterval()
  return 1
end
-- this function will be executed every frame on the server only1
function AITransferFinish.updateServer(timeStep)
  AITransferFinish.updateFlying(timeStep)
end
...
function getDockingPositionBack(s)
  sMatrix = s.position
  pos = sMatrix.translation
  dir = -sMatrix.look
  return pos, dir
...
function AITransferFinish.updateFlying(timeStep)
  if not craft then craft = Entity(player.craftIndex) end
  if not ship then ship = Entity() end
  AITransferFinish.flyfromShip(ship,craft)
...
--player ship
  ...
  --
  local craftPos, craftDir = getDockingPositionBack(craft)
  local craftPosBack = craftPos+craftDir  *craftRadius *2
  --
  local craftPosUp, craftDirUp =getDockingPositionUp(craft) 
  local craftPosAbove = craftCenter+craftDirUp * craftRadius* 2
  --
  ...
  if not waypoints or #waypoints == 0 then
    ...
    v4 = vec3(0,0,0)
    -- instanciate vec3
    v4 = craftPos + craftDir * far
    v3 = craftPos + craftDir * near
    v2 = craftPosBack
    v1 = craftPosAbove 
...
--    printTarget(v4)
  ...
  local dist= distance2(shipPos, waypoints[current])
